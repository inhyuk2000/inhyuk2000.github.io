...
## 코드 개선 이후
- 정답률 : {{% high_mark %}}**0.91**{{% /high_mark %}}

> [!warning] "매일 밤 10시부터 새벽 4시까지 조용히"

> [!warning] "매일 밤 10시부터 아침 6시까지 모든 알림 받지마"

- 주기성 명령의 경우 delivery와 expires 시간 계산에서 예측값과 실제값이 다르게 나옴. {{% high_mark %}}**하지만 이것이 결과에는 영향을 주지 않음**{{% /high_mark %}}
    - 프롬프트 변경을 통해 해결함.
        - 구간 전: 다음 회차
        - 구간 안: 진행 중 회차
        - 구간 끝남: 다음 밤 회차
        - 지난 회차 금지. `overrnight`면 `expires = delivery +  1`